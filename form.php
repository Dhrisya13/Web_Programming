<body bgcolor="#BCD8EC">
<?php
$name = $email = $password = $confirm_password = $dob = $phone = "";
$nameErr = $emailErr = $passwordErr = $confirmPasswordErr = $dobErr = $phoneErr = "";
$successMsg = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["phone"])) {
        ...
        if (!preg_match("/^[0-9]{10,15}$/", $phone)) {
            $phoneErr = "Phone number must be 10–15 digits";
        }
    }
    ...
    if (empty($nameErr) && empty($emailErr) && empty($passwordErr) &&
        empty($confirmPasswordErr) && empty($dobErr) && empty($phoneErr)) {
